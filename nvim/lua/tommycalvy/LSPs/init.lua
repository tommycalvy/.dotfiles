...
vim.api.nvim_create_autocmd('LspAttach', {
  ...
  callback = function(event)
    require('tommycalvy.LSPs.caps-on_attach').on_attach(vim.lsp.get_client_by_id(event.data.client_id), event.buf)
  end
})
      ...
      if require('nixCatsUtils').isNixCats then
        for server_name, cfg in pairs(servers) do
          require('lspconfig')[server_name].setup({
            capabilities = require('tommycalvy.LSPs.caps-on_attach').get_capabilities(server_name),
            -- this line is interchangeable with the above LspAttach autocommand
            -- on_attach = require('tommycalvy.LSPs.caps-on_attach').on_attach,
            settings = cfg,
